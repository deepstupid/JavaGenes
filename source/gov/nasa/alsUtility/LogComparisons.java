...
public class LogComparisons implements java.io.Serializable {
    ...
    public int getN(int i) {
        return up[i] + down[i] + between[i];
    }

    public double getUpFraction(int i) {
        if (getN(i) == 0) return -1;
        return (double) up[i] / (double) getN(i);
    }

    public double getDownFraction(int i) {
        if (getN(i) == 0) return -1;
        return (double) down[i] / (double) getN(i);
    }

    public double getBetweenFraction(int i) {
        if (getN(i) == 0) return -1;
        return (double) between[i] / (double) getN(i);
    }

    public double getUpFranction() {
        if (getN() == 0) return -1;
        return (double) getUp() / (double) getN();
    }

    public double getDownFranction() {
        if (getN() == 0) return -1;
        return (double) getDown() / (double) getN();
    }

    public double getBetweenFranction() {
        if (getN() == 0) return -1;
        return (double) getBetween() / (double) getN();
    }

    public int getN() {
        return Utility.arraySum(down) + Utility.arraySum(up) + Utility.arraySum(between);
    }

    public int getUp() {
        return Utility.arraySum(up);
    }

    public int getDown() {
        return Utility.arraySum(down);
    }

    public int getBetween() {
        return Utility.arraySum(between);
    }
    ...
    public String getHeaderFragment() {
        return "\tup\tdown\tbetween\ttotal\tupFraction\tdownFraction\tbetweenFraction";
    }
