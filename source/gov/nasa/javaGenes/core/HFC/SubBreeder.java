...
public class SubBreeder implements java.io.Serializable {
    ...
    protected void makeChildren(ChildMaker maker, int[] parentIndices, Population population) {
        ...
        for (int i = 0; i < c.length; i++) {
            ...
            if (debug) {
                Utility.debugPrintln(individual.toString());
            }
            if (shouldPromote(individual)) {
                promote(individual, population);
    ...
    public void newChild(Individual individual) {
        breeder.newChild(individual);
    }

    public int getTotalNumberOfKidsProduced() {
        return breeder.getTotalNumberOfKidsProduced();
    }

    public Evolvable getRandomEvolvable() {
        return breeder.getRandomEvolvable();
    }

    public void setPromotionsTo(SubBreeder subBreeder) {
        promotionsTo = subBreeder;
    }
