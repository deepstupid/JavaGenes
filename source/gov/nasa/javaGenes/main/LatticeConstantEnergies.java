...
public class LatticeConstantEnergies {
    ...
    public static void main(String[] arguments) {
        ...
        assertTrue(inputFile);
        outputDirectory = arguments[1];
        assertTrue(outputDirectory);
        Utility.makeDirectory(outputDirectory);
        parametersFile = arguments[2];
        assertTrue(parametersFile);
        numberOfSamples = Utility.string2integer(arguments[3]);
        assertTrue(numberOfSamples > 0, "numberOfSamples must be > 0");
        smallestUnitCellScale = Utility.string2double(arguments[4]);
        assertTrue(smallestUnitCellScale > 0);
        largestUnitCellScale = Utility.string2double(arguments[5]);
        assertTrue(largestUnitCellScale > smallestUnitCellScale);

        boolean doPerturbations = arguments.length >= perturbAlso;
        boolean doAtomsLimit = arguments.length >= limitAtomsToRandomize;

        if (doPerturbations) {
            numberOfPerturbedMolecules = Utility.string2integer(arguments[6]);
            assertTrue(numberOfPerturbedMolecules > 0);
            distanceToRandomize = Utility.string2double(arguments[7]);
            assertTrue(distanceToRandomize > 0);
        }
        if (doAtomsLimit)
            numberOfAtomsToRandomize = Utility.string2integer(arguments[8]);
        assertTrue(numberOfAtomsToRandomize > 0);
