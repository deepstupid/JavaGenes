...
public class TaskWeight extends gov.nasa.javaGenes.EOSscheduling.HBSS.TaskWeight {
    ...
    public void updateAndPropagate(int which) {
        ...
        switch (which) {
            case NEED:
                updateAndPropagateTaskNeed();
                return;
            case DIFFICULTY:
                updateAndPropagateTaskDifficulty();
                return;
            default:
                Error.fatal("bad case");
        }
    }
    ...
    public float getSensorAvailNeed() {
        return sensorAvailNeed.getCurrent();
    }

    public float getSensorAvailNeedFromCalculation() {
        return priority / (float) getContentionAWlist().currentSize();
    }

    public float getSSRneed() {
        return SSRneed.getCurrent();
    }

    public float getSSRneedFromCalculation() {
        return SSRuseTimesPriority / (float) getContentionAWlist().currentSize();
    }

    public float getSensorAvailDifficulty() {
        return sensorAvailDifficulty.getCurrent();
    }

    public float getSSRdifficulty() {
        return SSRdifficulty.getCurrent();
    }
    ...
    public int getSSRuse() {
        return ((TakeImage) task).getSSRuse();
    }

    public int getNumberOfUndecidedAccessWindows() {
        return getContentionAWlist().getNumberOfUndecidedAccessWindows();
    }

    public int getNumberOfAccessWindows() {
        return getContentionAWlist().getInitialSize();
    }
}
